...
def fetch_all_vietnam_stock_tickers() -> List[Dict[str, Any]]:
    """Lấy danh sách TOÀN BỘ hơn 1.500 cổ phiếu niêm yết trên 3 sàn HOSE, HNX, UPCoM"""
    ...
    # Filter only 3-letter equity stock symbols
    valid_stocks = []
    seen = set()
    for s in all_stocks:
    ...
    print(f"✅ Đã tải thành công danh sách TOÀN BỘ {len(valid_stocks):,} mã cổ phiếu trên HOSE, HNX, UPCoM!")
    return valid_stocks
...
def fetch_single_stock(s: Dict[str, Any], start_date: str, end_date: str) -> Optional[Dict[str, Any]]:
    sym = s["symbol"]
    try:
...
def sync_all_stocks_2025(max_limit: Optional[int] = None):
    start_time = time.time()
    print("================================================================================")
    print("🚀 BẮT ĐẦU THU THẬP & ĐỒNG BỘ DỮ LIỆU TOÀN BỘ CỔ PHIẾU VIỆT NAM NĂM 2025 VÀO POSTGRESQL")
    print("================================================================================\n")
    ...
    if max_limit:
        stocks_list = stocks_list[:max_limit]
    ...
    print(f"📈 Đang cào nến giá OHLCV năm 2025 cho {len(stocks_list):,} mã cổ phiếu (chạy đa luồng Parallel Workers)...")
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=5) as executor:
        future_to_stock = {
            executor.submit(fetch_single_stock, s, start_date, end_date): s
        ...
        for idx, future in enumerate(concurrent.futures.as_completed(future_to_stock), 1):
            ...
            try:
                ...
                if res and res["prices"]:
                    db.upsert_prices_2025(sym, res["prices"])
                    bars_count = len(res["prices"])
                    total_bars_synced += bars_count
                    success_count += 1
                    if idx % 20 == 0 or idx == len(stocks_list):
                        print(f"  [Progress: {idx}/{len(stocks_list)}] ✓ {sym:<5}: Đã lưu {bars_count} nến giá năm 2025.")
            except Exception as e:
                pass
                
    elapsed = time.time() - start_time
    print("\n================================================================================")
    print(f"🎉 HOÀN TẤT ĐỒNG BỘ DỮ LIỆU TOÀN BỘ CỔ PHIẾU VIỆT NAM NĂM 2025!")
    print(f"Tổng số mã cổ phiếu đã xử lý thành công: {success_count:,} / {len(stocks_list):,} mã.")
    print(f"Tổng số nến giá 2025 đã lưu trong PostgreSQL: {total_bars_synced:,} phiên.")
    print(f"Thời gian hoàn thành: {elapsed:.2f} giây.")
    print("Bạn có thể mở TablePlus kết nối PostgreSQL (localhost:5432) để truy vấn tables!")
    print("================================================================================\n")

if __name__ == "__main__":
    sync_all_stocks_2025()
