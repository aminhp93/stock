...
def process_and_enrich_prices(df: pd.DataFrame, target_start_date: str) -> List[Dict[str, Any]]:
    ...
    for idx, row in df.iterrows():
        ...
        # Chỉ lưu các nến giá từ target_start_date (2026-01-01) trở đi
...
def fetch_single_stock_2026(s: Dict[str, Any], warmup_start_date: str, target_start_date: str, end_date: str) -> Optional[Dict[str, Any]]:
    sym = s["symbol"]
    try:
        # Lấy từ 2025-10-01 để có đủ nến tính toán MA20, MA50 & RSI-14 chính xác cho đầu năm 2026
        df = stock_historical_data(sym, warmup_start_date, end_date, "1D")
        if df is not None and not df.empty:
            prices_data = process_and_enrich_prices(df, target_start_date)
            if prices_data:
...
def sync_2026_stock_history(target_start_date: str = "2026-01-01", warmup_start_date: str = "2025-10-01"):
    start_time = time.time()
    today_str = datetime.now().strftime("%Y-%m-%d")
    
    print("================================================================================")
    print(f"🚀 BẮT ĐẦU ĐỒNG BỘ DỮ LIỆU CỔ PHIẾU VIỆT NAM TỪ {target_start_date} ĐẾN NAY ({today_str})")
    print("================================================================ algorithm: Warmup MA20/MA50 từ " + warmup_start_date)
    print("================================================================================\n")
    ...
    print(f"📦 Đang cập nhật danh mục {len(stocks_list):,} mã cổ phiếu vào bảng 'stocks'...")
    db.upsert_stocks(stocks_list)
    print("✅ Đã cập nhật xong bảng danh mục 'stocks'.\n")
    
    print(f"📈 Đang cào nến giá OHLCV từ {target_start_date} đến {today_str} cho {len(stocks_list):,} mã (Parallel Workers)...")
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=8) as executor:
        future_to_stock = {
            executor.submit(fetch_single_stock_2026, s, warmup_start_date, target_start_date, today_str): s
            ...
            try:
                ...
                if res and res["prices"]:
                    ...
                    if idx % 50 == 0 or idx == len(stocks_list):
                        print(f"  [Progress: {idx}/{len(stocks_list)}] ✓ {sym:<5}: Đã lưu {bars_count} nến giá năm 2026.")
            except Exception as e:
                pass
                
    elapsed = time.time() - start_time
    print("\n================================================================================")
    print(f"🎉 HOÀN TẤT ĐỒNG BỘ DỮ LIỆU CỔ PHIẾU VIỆT NAM TỪ {target_start_date} ĐẾN {today_str}!")
    print(f"Tổng số mã cổ phiếu đã xử lý thành công: {success_count:,} / {len(stocks_list):,} mã.")
    print(f"Tổng số nến giá năm 2026 đã lưu trong PostgreSQL: {total_bars_synced:,} phiên.")
    print(f"Thời gian hoàn thành: {elapsed:.2f} giây.")
    print("================================================================================\n")

if __name__ == "__main__":
    sync_2026_stock_history()
