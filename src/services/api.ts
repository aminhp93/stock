...
export async function fetchStocks(): Promise<StockItem[]> {
  ...
  if (!res.ok) {
    const err = await res.json().catch(() => ({ error: 'Không thể kết nối cơ sở dữ liệu PostgreSQL (Bảng stocks).' }));
...
export async function fetchDataStats(): Promise<DataStats> {
  ...
  if (!res.ok) {
    const err = await res.json().catch(() => ({ error: 'Không thể kết nối cơ sở dữ liệu PostgreSQL.' }));
    throw new Error(err.error || `HTTP ${res.status}`);
  }
  return await res.json();
}
...
export async function fetchRawPrices(symbol: string, limit: number = 100): Promise<RawPricesResponse> {
  ...
  if (!res.ok) {
    const err = await res.json().catch(() => ({ error: `Không thể tải nến giá cho mã ${symbol} từ PostgreSQL.` }));
    throw new Error(err.error || `HTTP ${res.status}`);
  }
  return await res.json();
}
...
export async function syncStockData(symbol: string): Promise<SyncResult> {
  ...
  if (!res.ok) {
    const err = await res.json().catch(() => ({ error: `Không thể đồng bộ nến giá cho mã ${symbol}.` }));
    throw new Error(err.error || `HTTP ${res.status}`);
  }
  return await res.json();
}
...
export async function fetchChartData(symbol: string, startDate: string = '2021-01-01'): Promise<ChartResponse> {
  ...
  if (!res.ok) {
    const err = await res.json().catch(() => ({ error: `Không thể tải nến giá đồ thị cho mã ${symbol} từ PostgreSQL.` }));
    throw new Error(err.error || `HTTP ${res.status}`);
  }
  return await res.json();
}
...
export async function fetchStockSummary(symbol: string): Promise<StockSummary> {
  ...
  if (!res.ok) {
    const err = await res.json().catch(() => ({ error: `Không thể chạy pipeline phân tích cho mã ${symbol}.` }));
    throw new Error(err.error || `HTTP ${res.status}`);
  }
  return await res.json();
}
...
export async function runSimulation(symbol: string, count: number = 10000): Promise<SimulationResult> {
  ...
  if (!res.ok) {
    const err = await res.json().catch(() => ({ error: `Không thể chạy mô phỏng Monte Carlo cho mã ${symbol}.` }));
    throw new Error(err.error || `HTTP ${res.status}`);
  }
  return await res.json();
}
...
export async function fetchTelegramSentiment(symbol?: string): Promise<TelegramSentimentResult> {
  ...
  if (!res.ok) {
    const err = await res.json().catch(() => ({ error: 'Không thể phân tích sentiment Telegram.' }));
    throw new Error(err.error || `HTTP ${res.status}`);
  }
  return await res.json();
}
